...
class Tank(pygame.sprite.Sprite):
    ...
    def update(self, seconds):
        ...
        if pressedkeys[self.tankRightkey]:
            ...
            self.dy = 0
            self.tankAngle = 0
        elif pressedkeys[self.forwardkey]:
            self.dx = 0
            self.dy = -self.movespeed
            self.tankAngle = 90
        elif pressedkeys[self.tankLeftkey]:
            self.dx = -self.movespeed
            self.dy = 0
            self.tankAngle = 180
        elif pressedkeys[self.backwardkey]:
            self.dx = 0
            self.dy = self.movespeed
            self.tankAngle = 270
        # -------------- rotation ---------------
...
class SinglePlayerScene(SceneBase):
    ...
    def __init__(self):
        ...
        player_second = Tank((random.randint(0,Config.width), random.randint(0,Config.height)))
        for i in range(12):
...
class MultiBotsScene(SceneBase):
    def __init__(self):
        ...
        if self.client.room_id != None:
            ...
            self.Terminate()
        self.do_not_get_kicked = pygame.USEREVENT + 1
        pygame.time.set_timer(self.do_not_get_kicked, 25000)
        self.ranseq = ['UP', 'LEFT', 'DOWN', 'RIGHT']
    
    def ProcessInput(self, events, pressed_keys):
        enemy = []
        enemy_bullet = []
        for event in events:
            if event.type == self.do_not_get_kicked:
                self.client.tank_turn(self.client.token, random.choice(self.ranseq) )
        try:
            tanks = self.event_client.response['gameField']['tanks']
            bullets = self.event_client.response['gameField']['bullets']
            for tank in tanks:
                if tank['id'] == self.client.tank_id:
                    me = tank
                elif tank['id'] != self.client.tank_id:
                    enemy.append(tank)
            for bullet in bullets:
                if bullet['owner'] != self.client.tank_id:
                    enemy_bullet.append(bullet)
        except:
            pass
        for danger in enemy_bullet:
            if danger['direction'] == 'LEFT' or danger['direction'] == 'RIGHT':
                if danger['y'] in range( me['y']-16, me['y']+17 ):
                    self.client.tank_turn(self.client.token, 'DOWN')
                    pygame.time.set_timer(self.do_not_get_kicked, 25000)
            elif danger['direction'] == 'UP' or danger['direction'] == 'DOWN':
                if danger['x'] in range( me['x']-16, me['x']+17 ):
                    self.client.tank_turn(self.client.token, 'RIGHT')
                    pygame.time.set_timer(self.do_not_get_kicked, 25000)
        for target in enemy:
            if target['direction'] == 'LEFT' or target['direction'] == 'RIGHT':
                if target['y'] in range( me['y']-16, me['y']+17 ) and target['x'] not in range( me['x']-32, me['x']+34):
                    if target['x'] > me['x'] and me['direction'] == 'RIGHT':
                        self.client.tank_fire(self.client.token)
                        pygame.time.set_timer(self.do_not_get_kicked, 25000)
                    elif target['x'] > me['x'] and me['direction'] != 'RIGHT':
                        self.client.tank_turn(self.client.token, 'RIGHT')
                        self.client.tank_fire(self.client.token)
                        pygame.time.set_timer(self.do_not_get_kicked, 25000)
                    elif target['x'] < me['x'] and me['direction'] == 'LEFT':
                        self.client.tank_fire(self.client.token)
                        pygame.time.set_timer(self.do_not_get_kicked, 25000)
                    elif target['x'] < me['x'] and me['direction'] != 'LEFT':
                        self.client.tank_turn(self.client.token, 'LEFT')
                        self.client.tank_fire(self.client.token)
                        pygame.time.set_timer(self.do_not_get_kicked, 25000)
                elif target['y'] in range( me['y']-16, me['y']+17 ) and target['x'] in range( me['x']-32, me['x']+34):
                    self.client.tank_turn(self.client.token, 'DOWN')
                    pygame.time.set_timer(self.do_not_get_kicked, 25000)
            elif target['direction'] == 'UP' or target['direction'] == 'DOWN':
                if target['x'] in range( me['x']-16, me['x']+17 ) and target['y'] not in range( me['y']-32, me['y']+34):
                    if target['y'] > me['y'] and me['direction'] == 'DOWN':
                        self.client.tank_fire(self.client.token)
                        pygame.time.set_timer(self.do_not_get_kicked, 25000)
                    elif target['y'] > me['y'] and me['direction'] != 'DOWN':
                        self.client.tank_turn(self.client.token, 'DOWN')
                        self.client.tank_fire(self.client.token)
                        pygame.time.set_timer(self.do_not_get_kicked, 25000)
                    elif target['y'] < me['y'] and me['direction'] == 'UP':
                        self.client.tank_fire(self.client.token)
                        pygame.time.set_timer(self.do_not_get_kicked, 25000)
                    elif target['y'] < me['y'] and me['direction'] != 'UP':
                        self.client.tank_turn(self.client.token, 'UP')
                        self.client.tank_fire(self.client.token)
                        pygame.time.set_timer(self.do_not_get_kicked, 25000)
                elif target['x'] in range( me['x']-16, me['x']+17 ) and target['y'] in range( me['y']-32, me['y']+34):
                    self.client.tank_turn(self.client.token, 'RIGHT')
                    pygame.time.set_timer(self.do_not_get_kicked, 25000)
    ...
    def Update(self, seconds):
        ...
        for winner in winners:
            if self.client.tank_id == winner['tankId']:
                self.SwitchToScene( GameOverScreen(winner['score'], 1, True) )
                self.client.connection.close()
                self.event_client.channel.basic_cancel(consumer_tag = 'stop_this_please')
        for loser in losers:
            if self.client.tank_id == loser['tankId']:
                self.SwitchToScene( GameOverScreen(loser['score'], 2, True) )
                self.client.connection.close()
                self.event_client.channel.basic_cancel(consumer_tag = 'stop_this_please')
        for afk in kicked:
            if self.client.tank_id == afk['tankId']:
                self.SwitchToScene( GameOverScreen(afk['score'], 3, True) )
                self.client.connection.close()
                self.event_client.channel.basic_cancel(consumer_tag = 'stop_this_please')
    
    def Render(self, screen):
        background = pygame.Surface((screen.get_size()))
        background.fill((219,112,150)) 
        background = background.convert()
        screen.blit(background, (0, 0))
        leaderboard = []
        try:
...
class GameOverScreen(SceneBase):
    def __init__(self, final_score, reason, ai = False):
        SceneBase.__init__(self)
    ...
    def ProcessInput(self, events, pressed_keys):
        for event in events:
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_r and not self.ai:
                    self.SwitchToScene( MultiPlayerScene() )
                elif event.key == pygame.K_r and self.ai:
                    self.SwitchToScene( MultiBotsScene() )
